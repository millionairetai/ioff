...
//    const TYPE_CREATE_EDIT_PROJECT = 'edit_project';
    ...
    public static function getActivityWallByEmployeeId($employeeId, $offset, $perPage = 10, array $conditions = []) {
        ...
        $sql = " SELECT SQL_CALC_FOUND_ROWS employee.id AS employee_id, employee.firstname, employee.lastname, employee.profile_image_path,  " 
            ...
            .  "     LEFT JOIN task "  
            .  "         ON activity.owner_id = task.id AND activity.owner_table='task' AND (task.is_public=1 OR EXISTS(SELECT task_id FROM task_assignment WHERE task_id=task.id AND task_assignment.employee_id={$employeeId} AND task_assignment.disabled=0) OR EXISTS(SELECT task_id FROM follower WHERE task_id=task.id AND follower.employee_id={$employeeId} AND follower.disabled=0)) AND task.disabled=0 "
            .  "     LEFT JOIN project "
            .  "         ON activity.owner_id = project.id AND activity.owner_table='project' AND (project.is_public=1 OR EXISTS(SELECT project_id FROM project_employee WHERE project_id=project.id AND project_employee.employee_id={$employeeId} AND project_employee.disabled=0)) AND project.disabled=0 "
            .  "     LEFT JOIN event "
            .  "         ON activity.owner_id = event.id AND activity.owner_table='event' AND (event.is_public=1 OR EXISTS(SELECT event_id FROM invitee WHERE event_id=event.id AND invitee.employee_id={$employeeId} AND invitee.disabled=0)) AND event.disabled=0"
            ...
            .  "     LEFT JOIN event_post"
            .  "         ON activity.owner_id = event_post.id AND activity.owner_table='event_post' AND event_post.disabled=0 AND (EXISTS(SELECT * FROM event WHERE event.id=event_post.event_id and event.is_public=1 AND event.disabled=0) OR EXISTS(SELECT * FROM invitee WHERE invitee.event_id=event_post.event_id AND invitee.employee_id={$employeeId} AND invitee.disabled=0)) "
            .  "     LEFT JOIN event AS p_event "
            .  "         ON event_post.event_id=p_event.id AND p_event.disabled=0 "
            .  "     LEFT JOIN task_post "
